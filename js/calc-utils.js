...
(function () {
   ...
   * 勞退／勞保老年給付「平均餘命月數」簡表
   *
   * 法源：勞動部勞保局 113/4/1 公告「月退休金平均餘命表」（依內政部 111
   *       年簡易生命表）：60 歲=276 月（23 年）、65 歲=228 月（19 年）、
   *       70 歲=192 月（16 年）。https://www.bli.gov.tw/0018437.html
   *
   * TODO（P1，跨輪）：建立 LIFE_MONTHS_VERSIONS 版本化表、支援逐歲查表、
   *                 提供 getLifeMonths(age, { year }) 介面。
   *
   * 本函式為**所有頁面的平均餘命月數單一真實來源**，禁止在頁面內硬編複製。
   */
  function getLifeMonths(age) {
    if (age <= 60) return 276; // 23 年
    if (age <= 65) return 228; // 19 年
    if (age <= 70) return 192; // 16 年
    return 168;                // 70+ 保守 fallback
  }
   ...
   *   法源：全民健保法§18 及其附表（投保金額分級表授權條文）；衛生福利部 115/1/1 生效公告
